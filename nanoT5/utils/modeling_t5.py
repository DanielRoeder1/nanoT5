# Classes copied from transformers.models.t5.modeling_t5 + adjustments
import copy
import warnings
...
# [KFormer] -> added knowledge layers & forward logic, added layer_id in init, added knowledge_embeddings input
class T5LayerFF(nn.Module):
    def __init__(self, config: T5Config, layer_id: int):
        super().__init__()
        ...
        if layer_id in config.know_layer and config.is_decoder:
            self.know_k_fc = nn.Linear(config.know_dim,  config.d_model)
            self.know_v_fc = nn.Linear(config.know_dim,  config.d_model)
        else:
            self.know_k_fc = None
            self.know_v_fc = None

    def forward(self, hidden_states, knowledge_embeddings=None):
        hidden_states_norm = self.layer_norm(hidden_states)
        forwarded_states = self.DenseReluDense(hidden_states_norm)

        if knowledge_embeddings is not None and self.know_k_fc is not None:
            # Adjusting transposes from original -> https://github.com/zjunlp/Kformer/blob/main/fairseq/fairseq/modules/transformer_sentence_encoder_layer.py
            k = self.know_k_fc(knowledge_embeddings)
            v = self.know_v_fc(knowledge_embeddings)
            know = torch.matmul(hidden_states_norm, k.transpose(1,2))
            know = self.DenseReluDense.act(know)
            know = self.dropout(know)
            know = torch.matmul(know, v)
            forwarded_states = forwarded_states + know

        hidden_states = hidden_states + self.dropout(forwarded_states)
        return hidden_states

# [KFormer] -> added knowledge_embeddings input, layer_id in init -> send to T5LayerFF
class T5Block(nn.Module):
    ...
    def forward(
        ...
        # [KFormer] this is the last layer of the block i.e. MLP
        hidden_states = self.layer[-1](hidden_states, knowledge_embeddings=knowledge_embeddings)
...
# [KFormer] -> added knowledge_embedding input, layer_id to Block init
...
# [KFormer] -> added knowledge_embeddings input
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        # [KFormer]
        knowledge_embeddings: Optional[torch.FloatTensor] = None
...
class KnowledgeEncoder(nn.Module):
    ...
    def forward(self, enc_inputs):
        outs = self.enc(**enc_inputs)
        if self.pooling_strategy == 'cls':
            return self.cls_pooling(outs)
...
if __name__ == "__main__":
    ...
    config.know_layer = [8,9,10]
    config.know_dim = 768
    model = T5ForConditionalGeneration.from_pretrained(model_name, config = config)
    tokenizer = AutoTokenizer.from_pretrained(model_name)

    import torch
    inputs = tokenizer("this is a test", return_attention_mask=True, return_tensors="pt")
    inputs["knowledge_embeddings"] = torch.randn(1, 5, 768)
